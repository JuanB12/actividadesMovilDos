...
Modelo modeloGifFromJson(String str) => Modelo.fromJson(json.decode(str));

String modeloGifToJson(Modelo data) => json.encode(data.toJson());

class Gifs {
  List<Modelo> items = [];
  Gifs();
  Gifs.fromJson(jsonList) {
    if (jsonList == null) return;
    for (var elemento in jsonList['data']) {
      final gif = Modelo.fromJson(elemento);
      items.add(gif);
    }
  }
}
